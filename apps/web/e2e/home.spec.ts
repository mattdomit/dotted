...
test.describe("Home Page", () => {
  ...
  test('"How Dotted Works" shows 4 steps', async ({ page }) => {
    ...
    await expect(page.getByText("Restaurants Bid")).toBeVisible();
    await expect(page.getByText("Fresh & Served")).toBeVisible();
  });
  ...
  test("navigation links work", async ({ page }) => {
    await page.getByRole("link", { name: "Today's Vote" }).click();
